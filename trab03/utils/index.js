...
export const treatRawData = ({
  ...
  const treatedNodes = states.map((node) => {
    return {
      id: node,
      type: start.match(new RegExp(`^${node}$`))
        ? "start"
        : final.match(new RegExp(`^${node}$`))
        ? "end"
        : "",
      active: false,
    };
  ...
  for (let state of Object.keys(transitions)) {
    for (let value of Object.keys(transitions[state])) {
...
function getCurvature(source, target) {
...
function assembleLinks(
  ...
  if (
    ...
    treatedLinks.push({
      ...
      value: value,
      curvature: getCurvature(state, transitions[state][value]),
      active: false,
    });
  }
}
